...
public class RoomObjectController : MonoBehaviour
{
	...
	void Update ()
	{
		...
		//UPDATE: Updated to call new LoadScene Method
		if (GameOver) {
			GameOver = false;
			_theGameManager.LoadScene (GameManager.CurrentCharacterType.ToString () + "Frame", new Color (255, 255, 255, 0));
			// _theGameManager.FadeHandler(false, true, GameManager.CurrentCharacterType.ToString() + "Frame");//FadeOut();
		}
	}
